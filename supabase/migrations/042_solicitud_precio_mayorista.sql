-- ─────────────────────────────────────────────────────────────────────────────
-- 042: Solicitud de precio mayorista en pedidos
--
-- Caso operativo (Daniel): a veces clientes registrados como minorista
-- (con su lista de precios habitual) hacen un pedido grande y quieren
-- precio mayorista. El vendedor activa un botón en la PWA, los precios
-- del carrito se recalculan con la lista MAYORISTA, y el pedido llega
-- a la plataforma central marcado con un badge "Solicitud mayorista"
-- para que Daniel/operario decida si lo procesa con ese precio o no.
--
-- Diseño:
-- - pedidos.solicitud_mayorista BOOLEAN: marca el pedido como solicitud
--   especial de precio mayorista.
-- - pedidos.lista_precio_aplicada UUID: la lista que se usó al armar el
--   pedido (auditoria — útil para ver de qué lista vinieron los precios).
-- ─────────────────────────────────────────────────────────────────────────────

ALTER TABLE pedidos
  ADD COLUMN IF NOT EXISTS solicitud_mayorista BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS lista_precio_aplicada UUID REFERENCES listas_precio(id);

CREATE INDEX IF NOT EXISTS idx_pedidos_solicitud_mayorista
  ON pedidos (solicitud_mayorista)
  WHERE solicitud_mayorista = TRUE;

COMMENT ON COLUMN pedidos.solicitud_mayorista IS
  'TRUE si el vendedor solicitó tratar el pedido con precio mayorista, aunque el cliente esté registrado en otra lista. La plataforma central decide si lo procesa con ese precio o no.';
COMMENT ON COLUMN pedidos.lista_precio_aplicada IS
  'Lista de precios con la que se armó el pedido. Útil para auditoría cuando difiere de la lista habitual del cliente.';

-- Extender la RPC crear_pedido_atomico para aceptar los nuevos campos
CREATE OR REPLACE FUNCTION crear_pedido_atomico(
  p_pedido JSONB,
  p_items JSONB,
  p_permitir_sin_stock BOOLEAN DEFAULT FALSE,
  p_motivo_reposicion TEXT DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_pedido_id UUID;
  v_pedido_numero TEXT;
  v_item JSONB;
  v_pedido_row pedidos%ROWTYPE;
BEGIN
  IF p_items IS NULL OR jsonb_array_length(p_items) = 0 THEN
    RAISE EXCEPTION 'No se puede crear un pedido sin productos';
  END IF;

  INSERT INTO pedidos (
    numero, cliente_id, vendedor_id, fecha_pedido, fecha_despacho,
    estado, tipo_pago, direccion_entrega_id, direccion_entrega_texto,
    descuento_porcentaje, descuento_monto, subtotal, igv, incluir_igv,
    total, requiere_autorizacion, notas,
    requiere_reposicion, motivo_reposicion,
    solicitud_mayorista, lista_precio_aplicada
  ) VALUES (
    p_pedido->>'numero',
    (p_pedido->>'cliente_id')::UUID,
    NULLIF(p_pedido->>'vendedor_id', '')::UUID,
    (p_pedido->>'fecha_pedido')::DATE,
    (p_pedido->>'fecha_despacho')::DATE,
    COALESCE(p_pedido->>'estado', 'enviado')::estado_pedido,
    COALESCE(p_pedido->>'tipo_pago', 'contado'),
    NULLIF(p_pedido->>'direccion_entrega_id', '')::UUID,
    p_pedido->>'direccion_entrega_texto',
    COALESCE((p_pedido->>'descuento_porcentaje')::NUMERIC, 0),
    COALESCE((p_pedido->>'descuento_monto')::NUMERIC, 0),
    (p_pedido->>'subtotal')::NUMERIC,
    COALESCE((p_pedido->>'igv')::NUMERIC, 0),
    COALESCE((p_pedido->>'incluir_igv')::BOOLEAN, TRUE),
    (p_pedido->>'total')::NUMERIC,
    COALESCE((p_pedido->>'requiere_autorizacion')::BOOLEAN, FALSE),
    p_pedido->>'notas',
    p_permitir_sin_stock,
    CASE WHEN p_permitir_sin_stock THEN p_motivo_reposicion ELSE NULL END,
    COALESCE((p_pedido->>'solicitud_mayorista')::BOOLEAN, FALSE),
    NULLIF(p_pedido->>'lista_precio_aplicada', '')::UUID
  )
  RETURNING id, numero INTO v_pedido_id, v_pedido_numero;

  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    INSERT INTO pedidos_items (
      pedido_id, producto_id, lote_id, cantidad,
      precio_unitario, descuento_porcentaje, subtotal,
      force_reserva
    ) VALUES (
      v_pedido_id,
      (v_item->>'producto_id')::UUID,
      NULLIF(v_item->>'lote_id', '')::UUID,
      (v_item->>'cantidad')::NUMERIC,
      (v_item->>'precio_unitario')::NUMERIC,
      COALESCE((v_item->>'descuento_porcentaje')::NUMERIC, 0),
      (v_item->>'subtotal')::NUMERIC,
      p_permitir_sin_stock
    );
  END LOOP;

  SELECT * INTO v_pedido_row FROM pedidos WHERE id = v_pedido_id;
  RETURN jsonb_build_object(
    'id', v_pedido_row.id,
    'numero', v_pedido_row.numero,
    'requiere_reposicion', v_pedido_row.requiere_reposicion,
    'motivo_reposicion', v_pedido_row.motivo_reposicion,
    'solicitud_mayorista', v_pedido_row.solicitud_mayorista
  );
END;
$$;
